...
void forloop(char dir,queue_map* p_search_queue, map* p_curr_map, bst_map* p_explored_bst, cell_queue_map *p_curr_cell){

	map* p_new_map = move(p_curr_map, dir);
	int depth = (*p_curr_cell).depth;


	if(is_in_bst(*p_explored_bst, p_new_map)){
		if(p_new_map != p_curr_map){
			deallocate_map(p_new_map);
		}
	}else{
		insert(p_explored_bst, p_new_map);
...
stats* bst_solver(map* p_initial_map){
	...
	bst_map* p_explored_bst = (bst_map*)calloc(1, sizeof(bst_map));
	nil(p_explored_bst);
	...
	insert(p_explored_bst, p_initial_map); /*this bst make it possible to know if the search is in a good direction*/
	...
	while((!is_queue_empty(*p_search_queue))&&(!win)){
		...
		if(SokoWin(*p_curr_map)){
			...
			forloop('N', p_search_queue, p_curr_map, p_explored_bst, p_curr_cell);
			forloop('S', p_search_queue, p_curr_map, p_explored_bst, p_curr_cell);
			forloop('E', p_search_queue, p_curr_map, p_explored_bst, p_curr_cell);
			forloop('W', p_search_queue, p_curr_map, p_explored_bst, p_curr_cell);
		}
	}
